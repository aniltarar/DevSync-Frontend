  ...
  <div>
    ...
    <v-row class="mb-2">
      <v-col v-for="card in statCards" :key="card.title" cols="6" sm="4" lg>
        <v-card
          ...
          :class="card.value > 0 && card.highlight ? 'border-opacity-100' : ''"
          :style="
            card.value > 0 && card.highlight
              ? `border: 1px solid rgb(var(--v-theme-${card.color}))`
              : ''
          "
        >
          <div class="pa-4">
            <div class="d-flex align-center justify-space-between mb-3">
              ...
              <v-chip
                v-if="card.highlight && card.value > 0"
                :color="card.color"
                size="small"
                variant="flat"
              >
    ...
    <v-row>
      ...
      <v-col cols="12" md="6">
        <v-card rounded="xl">
          <v-card-title
            class="d-flex align-center justify-space-between pa-4 pb-0"
          >
                ...
                <template #prepend>
                  ...
                  >
                    <v-img
                      ...
                      :src="getMediaUrl(user.profile.avatarUrl)"
                    />
          ...
          <v-card-text class="pa-2">
            <v-list lines="two">
              ...
              >
                <template #prepend>
                  <v-avatar
                    :color="reportStateColor(report.status?.state)"
                    variant="tonal"
                    size="40"
                    class="mr-3"
                  >
                    <v-icon size="20">{{
                      reportStateIcon(report.status?.state)
                    }}</v-icon>
                  </v-avatar>
                </template>
                <template #title>
                  <div class="d-flex align-center ga-2">
                    <span class="text-body-2 font-weight-medium">{{
                      report.reporterId?.username || "Anonim"
                    }}</span>
                ...
                <template #subtitle>
                  <span class="text-caption"
                    >{{ report.reason ? reasonLabel(report.reason) + " · " : ""
...
const statCards = computed(() => [
  ...
  {
    title: "Gönderiler",
    value: adminStore.stats?.totalPosts || 0,
    icon: "mdi-post-outline",
    color: "info",
  },
  {
    title: "Projeler",
    value: adminStore.stats?.totalProjects || 0,
    icon: "mdi-folder-outline",
    color: "success",
  },
  {
    title: "Yorumlar",
    value: adminStore.stats?.totalComments || 0,
    icon: "mdi-comment-outline",
    color: "secondary",
  },
  {
    title: "Bekleyen Raporlar",
    value: adminStore.stats?.pendingReports || 0,
